...
class Handler():
	...
	def create_addresses(self):
		"""
		Create user definied number of addresses, 10 by default.
		"""
		self.addresses = []
		print('\nReceiving addresses:')
		...
		try:
			while True:
				...
				try:
					...
					print('\tDenomination: {}'.format(cj_info['denomination']))
					print('\tRegistration ends at: {}'.format(cj_info['inputRegistrationTimesout']))
					print('\tPeers registered: {}'.format(cj_info['registeredPeerCount']))
					print('\tCoordinator fee % per AS: {}'.format(cj_info['coordinatorFeePercent']))
